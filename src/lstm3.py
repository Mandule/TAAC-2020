    ...
    def collate_fn(self, batch_data):
        if self.tag == 'train':
            for data in batch_data:
                user_seqs = data[0]
                index = np.arange(user_seqs[0].shape[0])
                index_drop = np.random.choice(index.shape[0], int(index.shape[0]*0.05), replace=False)
                index = np.delete(index, index_drop)
                for i in range(self.num_seqs):
                    user_seqs[i] = user_seqs[i][index]
...
class classfiyNet(nn.Module):
    def __init__(self, input_size, num_labels, drop_out):
        ...
        self.fc = nn.Sequential(
            ...
            nn.Dropout(drop_out),
            nn.LeakyReLU(inplace=True),
            nn.Linear(input_size // 4, num_labels)
        )
        self.init_params()
...
# 完整的网路结构
class Net(nn.Module):
    def __init__(self, embed_size, fea_size, hidden_size, num_layers, drop_out):
        ...
        self.init_params()
        self.fc_output = classfiyNet(4 * hidden_size, 20, drop_out)
